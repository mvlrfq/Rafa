...
local DOWNLOAD_FOLDER = FOLDER_NAME .. "/Downloaded" -- Sub-folder khusus hasil download link
local PLAYLIST_FILE = FOLDER_NAME .. "/playlist_data.json"

-- Buat folder utama dan sub-folder download jika belum ada
if makefolder then
    if not isfolder(FOLDER_NAME) then
        makefolder(FOLDER_NAME)
    end
    if not isfolder(DOWNLOAD_FOLDER) then
        makefolder(DOWNLOAD_FOLDER)
    end
end
...
local function loadPlaylistFromLocal()
    if isfile and isfile(PLAYLIST_FILE) and readfile then
        local success, result = pcall(function()
            ...
            return HttpService:JSONEncode(fileContent)
        end)
...
-- 6. LOGIC DOWNLOAD LINK (Disimpan di Sub-Folder: Downloaded)
addBtn.MouseButton1Click:Connect(function()
    ...
    if string.sub(src, 1, 4) == "http" then
        ...
        statusLabel.Text = "⏳ Men-download ke folder Downloaded..."

        task.spawn(function()
            -- Simpan file ke sub-folder DOWNLOADED secara khusus
            ...
            if downloadSuccess and audioData and #audioData > 0 then
                ...
                nameBox.Text = ""
                refreshPlaylistUI()
                statusLabel.Text = "✅ Link tersimpan di folder Downloaded!"
            else
...
-- LOGIC SCAN FILE LOKAL (Hanya memindai Folder Utama MyMusicPlayer)
scanBtn.MouseButton1Click:Connect(function()
    ...
    for _, filePath in ipairs(files) do
        -- Hanya scan file MP3 yang terletak di folder utama (mengabaikan sub-folder Downloaded)
    ...
    if countAdded > 0 then
        savePlaylistToLocal()
        refreshPlaylistUI()
        statusLabel.Text = "Scan sukses! +" .. countAdded .. " MP3 Lokal"
    else
