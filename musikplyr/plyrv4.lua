...
-- Memuat playlist yang tersimpan saat pertama kali dijalankan
...
local isDownloading = false -- Flag indikator status download
...
playTrack = function(index)
    ...
    -- Opsi 1: File MP3 Lokal Langsung
    if track.Type == "Local" or (isfile and isfile(track.Source)) then
        local success, result = pcall(function()
        ...
        if success and result then
            ...
            sound:Play()
            statusLabel.Text = "Playing (Offline): " .. track.Name
        else
            statusLabel.Text = "Gagal memuat file lokal!"
        end
        
    -- Opsi 2: Link URL HTTP MP3 (Dilakukan saat tombol Tambah diklik)
    elseif string.sub(track.Source, 1, 4) == "http" then
        local success, result = pcall(function()
            return getcustomasset and getcustomasset(track.Source)
        end)
...
-- 6. Event Logic Tombol Tambah dengan FITUR LOADING DOWNLOAD
addBtn.MouseButton1Click:Connect(function()
    if isDownloading then return end -- Cegah spam klik saat sedang download
    ...
    -- Cek jika input berupa Link HTTP (Memerlukan Download)
    if string.sub(src, 1, 4) == "http" then
        isDownloading = true
        addBtn.Text = "⏳ Men-download..."
        addBtn.BackgroundColor3 = Color3.fromRGB(150, 100, 20)
        statusLabel.Text = "⏳ Men-download audio dari internet..."

        -- Gunakan task.spawn agar UI tidak mendadak freeze saat men-download
        task.spawn(function()
            local fileName = FOLDER_NAME .. "/track_" .. (os.time()) .. ".mp3"
            ...
            if downloadSuccess and audioData and #audioData > 0 then
                ...
                table.insert(playlist, {Name = name, Source = fileName, Type = "Local"})
                savePlaylistToLocal()
                
                urlBox.Text = ""
                nameBox.Text = ""
                refreshPlaylistUI()
                statusLabel.Text = "✅ Download Selesai & Disimpan!"
            else
                statusLabel.Text = "❌ Gagal download dari link tersebut!"
            end
        ...
        -- Jika hanya Roblox Sound ID biasa
        ...
        nameBox.Text = ""
        refreshPlaylistUI()
        statusLabel.Text = "Berhasil ditambahkan!"
    end
end)

-- LOGIC SCAN FILE LOKAL (.mp3)
scanBtn.MouseButton1Click:Connect(function()
    ...
    for _, filePath in ipairs(files) do
        if string.sub(filePath, -4):lower() == ".mp3" then
    ...
    if countAdded > 0 then
        savePlaylistToLocal()
        refreshPlaylistUI()
        statusLabel.Text = "Berhasil scan + " .. countAdded .. " file MP3!"
    else
        statusLabel.Text = "Tidak ada MP3 baru di folder!"
    end
end)
