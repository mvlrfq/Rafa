...
local loopDelay = 2
local flySpeed = 50 -- Kecepatan terbang default (Studs per detik)
...
-- Fungsi Terbang Smooth (Tweening) ke Lokasi Tujuan
local function flyTo(cframe)
    ...
    local timeToTravel = distance / flySpeed

    -- Menonaktifkan gravitasi sementara & membuat tidak jatuh
    ...
    -- Batal terbang jika fitur dimatikan di tengah jalan
    local connection
    connection = game:GetService("RunService").Heartbeat:Connect(function()
        if not autoFlyActive then
            tween:Cancel()
...
WaypointTopControls.Size = UDim2.new(1, 0, 0, 135)
...
WaypointDivider.Position = UDim2.new(0, 0, 0, 140)
...
WaypointListTitle.Position = UDim2.new(0, 2, 0, 145)
...
WaypointListBox.Size = UDim2.new(1, -6, 1, -168)
WaypointListBox.Position = UDim2.new(0, 0, 0, 165)
...
-- FITUR BARU: AUTO FLY TELEPORT (LOOPING)
createToggle(WaypointTopControls, "Auto Fly to wp (Looping)", false, function(Value)
    autoFlyActive = Value
    showNotification("Auto Fly", "Auto Fly To wp: " .. (Value and "ON" or "OFF"), 2)
    task.spawn(function()
        while autoFlyActive do
            if #customWaypoints == 0 then break end
            for _, wp in ipairs(customWaypoints) do
                if not autoFlyActive then break end
                flyTo(wp.CF)
                task.wait(loopDelay)
            end
            task.wait(0.5)
...
-- FITUR BARU: Pengaturan Kecepatan Terbang
createSliderWithInput(PageSettings, "Kecepatan Terbang (Fly Speed)", 10, 200, flySpeed, function(Value)
    flySpeed = Value
    showNotification("Setting Updated", "Kecepatan terbang: " .. Value .. " Studs/s", 1.5)
end)
